...
// A scheduled fight is "predictable" only if BOTH fighters exist in FIGHTER_DB.
// The auto-updater seeds cards with "TBA" / "Opponent TBA" placeholders that
// aren't in the DB — landing on one of those used to crash the whole app.
function isPredictableFight(f) {
  return !!(f && FIGHTER_DB[f.f1] && FIGHTER_DB[f.f2]);
}

// Find the first upcoming event+fight where both fighters are in the DB, so the
// app opens on a real matchup instead of an empty placeholder. Falls back to 0,0.
function firstPredictableIndex() {
  for (let ei = 0; ei < UPCOMING_EVENTS.length; ei++) {
    const fights = UPCOMING_EVENTS[ei]?.fights || [];
    for (let fi = 0; fi < fights.length; fi++) {
      if (isPredictableFight(fights[fi])) return { ei, fi };
    }
  }
  return { ei: 0, fi: 0 };
}
const DEFAULT_SEL = firstPredictableIndex();
...
export default function App(){
  ...
  const[selEvt,setSelEvt]=useState(DEFAULT_SEL.ei); // first card with a real matchup
  const[selFight,setSelFight]=useState(DEFAULT_SEL.fi);
  ...
  const evtMeta=UPCOMING_EVENTS[selEvt]||{fights:[]};
  const fightMeta=evtMeta.fights?.[selFight]||{f1:"",f2:"",context:""};
        ...
        <div style={{height:"calc(100vh - 57px)",overflowY:"auto"}}>
          ...
          <div style={{overflowX:"auto",borderBottom:"1px solid #1a1a1a",padding:"8px 12px",display:"flex",gap:6,WebkitOverflowScrolling:"touch"}}>
            {UPCOMING_EVENTS.map((evt,ei)=>
              (evt.fights||[]).map((f,fi)=>{
                if(!isPredictableFight(f)) return null; // skip TBA / unknown fighters
          ...
          <div style={{padding:"16px 16px"}}>
            ...
            {(!f1||!f2)?(
              <div style={{textAlign:"center",padding:"48px 20px",color:"#5a5a6a",fontSize:13,lineHeight:1.6}}>
                <div style={{fontSize:32,marginBottom:12}}>🥊</div>
                No predictable matchups scheduled yet.<br/>
                Fighters for upcoming cards are still <strong style={{color:"#d4a843"}}>TBA</strong> —
                check back once bouts are announced, or try the <strong style={{color:"#d4a843"}}>Search</strong> and <strong style={{color:"#d4a843"}}>Fantasy</strong> tabs.
              </div>
            ):(
              <>
